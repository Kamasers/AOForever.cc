            ...
            If .TileExit.map > 0 And .TileExit.map <= NumMaps Then
                ...
                ElseIf UCase$(MapInfo(DestPos.map).Restringir) = "FACCION" Then '¿Es mapa de faccionarios?
                    ...
                    Else 'No es Faccionario
                        ...
                        End If
                    End If
                ElseIf DestPos.map = 175 Then
                    If UserList(UserIndex).Stats.ELV < 40 Then
                        Call WriteConsoleMsg(UserIndex, "Mapa exclusivo para niveles superiores a 40.", FontTypeNames.FONTTYPE_INFO)
                        Call ClosestStablePos(UserList(UserIndex).Pos, nPos)
        ...
        With .flags
            ...
            If FoundSomething = 1 Then
                ...
                If MostrarCantidad(.TargetObj) Then
                    Call WriteConsoleMsg(UserIndex, ObjData(.TargetObj).Name, FontTypeNames.FONTTYPE_INFO) 'Call WriteConsoleMsg(UserIndex, ObjData(.TargetObj).name & " - " & MapData(.TargetObjMap, .TargetObjX, .TargetObjY).ObjInfo.Amount & "", FontTypeNames.FONTTYPE_INFO)
                Else
                    Call WriteConsoleMsg(UserIndex, ObjData(.TargetObj).Name, FontTypeNames.FONTTYPE_INFO)
                End If
                If .TargetObj = mod_HungerGames.COFRE_CERRADO_OBJINDEX Then
                    Call mod_HungerGames.AbrirCofre(UserIndex, map, .TargetObjX, .TargetObjY, False)
                ElseIf .TargetObj = mod_HungerGames.COFRE_ABIERTO_OBJINDEX Then
                    Call mod_HungerGames.AbrirCofre(UserIndex, map, .TargetObjX, .TargetObjY, True)
                End If
            End If
            '¿Es un personaje?
