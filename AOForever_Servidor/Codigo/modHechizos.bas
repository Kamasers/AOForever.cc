...
With UserList(UserIndex)
    ...
    ElseIf Hechizos(Spell).SubeHP = 2 Then
        ...
        If .flags.Privilegios And PlayerType.User Then
            ...
            .Stats.MinHp = .Stats.MinHp - daño
            SendData SendTarget.ToPCArea, UserIndex, PrepareMessageCreateDamage(.Pos.X, .Pos.Y, "-" & daño, 255, 0, 0)
            Call WriteConsoleMsg(UserIndex, Npclist(NpcIndex).name & " te ha quitado " & daño & " puntos de vida.", FontTypeNames.FONTTYPE_FIGHT)
            Call WriteUpdateUserStats(UserIndex)
...
With UserList(UserIndex)
    ...
    ' Invocacion normal
    Else
        If SpellIndex = 29 Then
            If .NroMascotas = 1 Then Exit Sub
        End If
...
With UserList(UserIndex)
    ...
    If Hechizos(HechizoIndex).Paraliza = 1 Or Hechizos(HechizoIndex).Inmoviliza = 1 Then
        ...
        If UserList(UserIndex).UserReto.MandoReto.AIM = True Or UserList(TargetIndex).UserReto.MandoReto.AIM = True Then
            Call WriteConsoleMsg(UserIndex, "No podés usar inmovilizar en un reto al AIM.", FontTypeNames.FONTTYPE_FIGHT)
            Exit Sub
        End If
...
With Npclist(NpcIndex)
    ...
    ElseIf Hechizos(SpellIndex).SubeHP = 2 Then
        ...
        If UserList(UserIndex).Invent.AnilloEqpObjIndex = LAUDELFICO Or UserList(UserIndex).Invent.AnilloEqpObjIndex = FLAUTAELFICA Or .Invent.AnilloEqpObjIndex <> ANILLOMAGICO Then
        ...
        .Stats.MinHp = .Stats.MinHp - daño
         SendData SendTarget.ToNPCArea, NpcIndex, PrepareMessageCreateDamage(.Pos.X, .Pos.Y, "-" & daño, 255, 0, 0)
...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        If tUser > 0 Then
            ...
            End If
        ElseIf tNPC > 0 Then
            If Hechizos(SpellIndex).GrhTravel > 0 Then
                Call SendData(SendTarget.ToNPCArea, tNPC, PrepareMessageCreateProjectile(UserList(UserIndex).Char.CharIndex, Npclist(tNPC).Char.CharIndex, Hechizos(SpellIndex).GrhTravel))
            End If
...
With UserList(TargetIndex)
    ...
    ElseIf Hechizos(SpellIndex).SubeHP = 2 Then
        ...
        Call WriteUpdateHP(TargetIndex)
        SendData SendTarget.ToPCArea, TargetIndex, PrepareMessageCreateDamage(.Pos.X, .Pos.Y, "-" & daño, 255, 0, 0)
