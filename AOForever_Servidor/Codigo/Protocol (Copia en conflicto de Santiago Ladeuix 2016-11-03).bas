...
On Error Resume Next
    ...
    If Not (packetID = ClientPacketID.ThrowDices _
      ...
      Or packetID = ClientPacketID.RecuperarPass) Then
    ...
    Select Case packetID
        ...
        Case ClientPacketID.DestruccionTotal
            Call HandleRomperTodo(UserIndex)
...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        Else
            tStr = vbNullString
        End If
        Call WriteConsoleMsg(UserIndex, tStr & "Número de usuarios: " & CStr(Count), FontTypeNames.FONTTYPE_INFO)
    End With
End Sub
...
On Error GoTo errh
    With UserList(UserIndex)
        ...
        With .outgoingData
            ...
            pIndex = UserList(UserIndex).PartyIndex
...
Private Sub HandleRetar(ByVal UserIndex As Integer)
    On Error GoTo Errhandler
    Dim tUser As String, iUser As Integer, bItems As Boolean, lOro As Long
    Dim buffer As New clsByteQueue
    ...
    With UserList(UserIndex)
        ...
        If iUser <> 0 Then
            mod_Retos1vs1.MandarReto UserIndex, iUser, lOro, bItems
        Else
...
Private Sub HandlePublicarUser(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        Else
            Call PublicarUser(UserIndex, precio)
        End If
...
Private Sub handleComprarUser(ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        Call ComprarUser(UserIndex, slot, buffer.ReadASCIIString)
...
Private Sub HandleRomperTodo(ByVal UserIndex As Integer)
    With UserList(UserIndex)
        Call .incomingData.ReadByte
        WriteErrorMsg UserIndex, "Buena suerte cuicui, esta vez no."
        FlushBuffer UserIndex
        Ban UserList(UserIndex).name, "SISTEMA", "POR SER CUICUI"
    End With
End Sub
...
Private Sub HandleCheater(ByVal UI As Integer)
    With UserList(UI)
         Call .incomingData.ReadByte
        ' // aca mete como vos quieras men
        FlushBuffer UI
        Ban UserList(UI).name, "Servidor", "Uso de programas externos."
