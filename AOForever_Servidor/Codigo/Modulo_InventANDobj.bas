...
On Error GoTo Errhandler
    ...
    End If
    Call mod_Limpieza.AgregarObj(Pos.map, NuevaPos.x, NuevaPos.Y)
        ...
        For NroDrop = 1 To 5
            ...
            If ObjIndex > 0 Then
                ...
                If ObjIndex = iORO Then
                    Call TirarOroNpc(.Drop(NroDrop).Amount * OroMulti, npc.Pos)
                    ''UserList(UserIndex).Stats.GLD = UserList(UserIndex).Stats.GLD + .Drop(NroDrop).Amount * OroMulti
                    ''Call WriteUpdateGold(UserIndex)
                    ''Call WriteConsoleMsg(UserIndex, "La criatura ha dejado " & .Drop(NroDrop).Amount * OroMulti & " monedas de oro.", FontTypeNames.FONTTYPE_INFO)
                Else
            ...
            End If
        Next NroDrop
        If .GiveGLD > 0 Then Call TirarOroNpc(.GiveGLD, npc.Pos)
