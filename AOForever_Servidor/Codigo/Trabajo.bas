...
Option Explicit
Public Const TIEMPO_INICIOMEDITAR As Integer = 640
...
Dim OtroUserIndex As Integer
    With UserList(UserIndex)
        If .flags.Comerciando Then
            ...
            End If
        End If
        CantidadItems = MaximoInt(1, CInt((.Stats.ELV - 4)))
...
On Error GoTo Errhandler
    ...
    With UserList(UserIndex)
        If .NroMascotas < MAXMASCOTAS Then
            ...
            ' 11% de bonificacion
            ElseIf .Invent.AnilloEqpObjIndex = FLAUTAMAGICA Or .Invent.AnilloEqpObjIndex = ANILLOMAGICO Then
...
If RandomNumber(0, 100) < Suerte Then
    If VictimUserIndex <> 0 Then
        ...
        SendData SendTarget.ToPCArea, VictimUserIndex, PrepareMessageCreateDamage(UserList(VictimUserIndex).Pos.X, UserList(VictimUserIndex).Pos.Y, "-" & daño, 210, 210, 210)
        Call FlushBuffer(VictimUserIndex)
    Else
        Npclist(VictimNpcIndex).Stats.MinHp = Npclist(VictimNpcIndex).Stats.MinHp - Int(daño * 2)
        SendData SendTarget.ToNPCArea, VictimNpcIndex, PrepareMessageCreateDamage(Npclist(VictimNpcIndex).Pos.X, Npclist(VictimNpcIndex).Pos.Y, "-" & daño, 210, 210, 210)
...
If res <= 10 Then ''6
    ...
    If UserList(UserIndex).clase = eClass.Worker Then
        With UserList(UserIndex)
            CantidadItems = 1 + MaximoInt(1, Skill / 2)
        End With
...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        If .Counters.tInicioMeditar > 0 Then
            .Counters.tInicioMeditar = .Counters.tInicioMeditar - 40
            Exit Sub
        End If
        ...
        If GetTickCount - .LastMedit < 500 Then
            Exit Sub
        Else
            .LastMedit = GetTickCount
        End If

        res = RandomNumber(1, Suerte)
        
        'If res = 1 Then
            ''cant = Porcentaje(UserList(UserIndex).Stats.MaxMAN, Round(MeditarSkill / 20))
            cant = (UserList(UserIndex).Stats.MaxMAN / 100) * (MeditarSkill / 20)
