...
Sub ConnectNewUser(ByVal UserIndex As Integer, ByRef Name As String, ByRef Password As String, ByVal UserRaza As eRaza, ByVal UserSexo As eGenero, ByVal UserClase As eClass, _
                    ByRef UserEmail As String, ByVal Hogar As eCiudad, ByVal Head As Integer, ByVal pin As String)
'*************************************************
'Author: Unknown
...
With UserList(UserIndex)
    ...
    .Stats.MaxHp = RandomNumber(20, 21) ''15 + MiInt
    .Stats.MinHp = .Stats.MaxHp ''15 + MiInt
    ...
    Select Case UserClase
        Case eClass.Hunter
            ' Arco (Newbie)
            .Invent.Object(slot).ObjIndex = 845
        Case eClass.Worker
            ' Herramienta (Newbie)
    ...
    If UserClase = eClass.Hunter Then
        slot = slot + 1
        .Invent.Object(slot).ObjIndex = 844
        .Invent.Object(slot).Amount = 150
        
        ' Equipo flechas
        .Invent.Object(slot).Equipped = 1
        .Invent.MunicionEqpSlot = slot
        .Invent.MunicionEqpObjIndex = 844
    End If
...
Call WriteVar(CharPath & UCase$(Name) & ".chr", "BALANCEP", "Actualizados", "1")
...
On Error GoTo Errhandler
    ...
    If UserList(UserIndex).UserReto.EnReto = True Then
        Dim tWINN As Integer
        Dim nArena As Byte
        nArena = UserList(UserIndex).UserReto.Arena
        If nArena > 0 Then
            If Arena(nArena).Player(0).UserIndex = UserIndex Then tWINN = Arena(nArena).Player(1).UserIndex Else _
            tWINN = Arena(nArena).Player(0).UserIndex
            
            TerminarReto tWINN, UserIndex
        End If
    End If
    
    If mod_DeathMatch.Death.Activo Then
        If UserList(UserIndex).UserDeath.SecondsBack > 0 Then
            WarpUserChar UserIndex, UserList(UserIndex).UserDeath.lastPos.map, UserList(UserIndex).UserDeath.lastPos.X, UserList(UserIndex).UserDeath.lastPos.Y, True
            UserList(UserIndex).UserDeath.SecondsBack = 0
            mod_DeathMatch.Death.Activo = False
            mod_DeathMatch.Death.Comenzado = False
            mod_DeathMatch.Death.CuentaRegresiva = 10
        End If
    End If
    
    If mod_HungerGames.Hunger_Games.Active = True Then
         If UserList(UserIndex).HungerGames.SecondsBack > 0 Then
            WarpUserChar UserIndex, UserList(UserIndex).HungerGames.lastPos.map, UserList(UserIndex).HungerGames.lastPos.X, UserList(UserIndex).HungerGames.lastPos.Y, True
            UserList(UserIndex).UserDeath.SecondsBack = 0
            mod_HungerGames.Hunger_Games.Active = False
            mod_HungerGames.Hunger_Games.Starting = False
            mod_HungerGames.Hunger_Games.CountDown = 10
        End If
    End If
    
    Call DeslogeaParty(UserIndex)
    Call eventClose(UserIndex)
    
    If UserList(UserIndex).EventAim.ID_Array > 0 Then _
        Call Event_1vs1_Aim_Melee.Disconnect_User(UserIndex)
    
    If UserList(UserIndex).UserDeath.EnDeath = True Then _
        Call mod_DeathMatch.Muere_Death(UserIndex, True)
        
    If UserList(UserIndex).HungerGames.HungerGamers = True Then _
        Call Muere_HungerGames(UserIndex, True)
    
    If UserList(UserIndex).Retos3vs3.Arena > 0 Then _
        Call mod_Retos3vs3.Cancel_Reto(UserIndex)
 
    If UserList(UserIndex).Retos3vs3.players(1) = UserIndex Then _
        Call mod_Retos3vs3.Cancel_Send(UserIndex, UserIndex)
 
    If UserList(UserIndex).Retos3vs3.ID_Send > 0 Then _
        Call mod_Retos3vs3.Cancel_Send(UserList(UserIndex).Retos3vs3.ID_User_Send, UserIndex)

    If UserList(UserIndex).Torneo.EnTorneo = True Then _
        Call mod_T1v1.userSaleTorneo(UserIndex)
...
With UserList(UserIndex)
    ...
    If GetVar(CharPath & UCase$(Name) & ".chr", "Mercado", "Vendiendo") = "1" Then
        Call WriteErrorMsg(UserIndex, "Este personaje se encuentra a la venta, si es de tu pertenencia lo puedes sacar de la venta desde otro personaje")
        Call FlushBuffer(UserIndex)
        Call CloseSocket(UserIndex)
        Exit Sub
    End If
    ...
    ''If (val(GetVar(CharPath & UCase$(.name) & ".chr", "BALANCEP", "Actualizados")) = 0) Then
        ''Dim hp As Integer, Promedio As Double
      ''  Dim x As Long
        
        ''Promedio = ModVida(.clase) - (21 - (.Stats.UserAtributos(eAtributos.Constitucion))) * 0.5
       '' If Not .Stats.ELV = 1 Then
           '' For x = 1 To (.Stats.ELV - 1)
            ''    hp = hp + Round(Promedio + gethp(Promedio))
            ''Next x
            ''hp = hp + 20
            ''.Stats.MinHp = hp
            ''.Stats.MaxHp = hp
    ...
    End If
    Call GenerateOnlineString
    
    If OnlineNum > recordusuarios Then
...
UserList(UserIndex).EnEvento = False
UserList(UserIndex).Torneo.EnTorneo = False
UserList(UserIndex).Torneo.SuCupo = 0

UserList(UserIndex).Captura.EnCaptura = False
UserList(UserIndex).Captura.Team = 0
With UserList(UserIndex).UserReto
    ...
    .MandoReto.tIndex = 0
End With
With UserList(UserIndex).UserDeath
    .EnDeath = False
    .SecondsBack = 0
End With
With UserList(UserIndex).EventAim
    .ID_Array = 0
End With
With UserList(UserIndex).Freeze
    .Freezing = False
    .ID_Team_Array = 0
    .ID_User_Array = 0
End With
With UserList(UserIndex).HungerGames
    .HungerGamers = False
