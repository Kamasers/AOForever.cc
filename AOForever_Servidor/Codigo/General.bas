...
On Error Resume Next
    ...
    ChDir App.path
    ChDrive App.path
    Call Event_1vs1_Aim_Melee.Load_Arenas
    Call mod_Retos3vs3.Load_Arenas
    Call LoadMotd
    Call BanIpCargar
    Call eventLoad
    Call InitRetos
    ...
    Call InitMercado
    Call CargarRanking
    Call LoadHungerGames
    Loteria.id_Loteria = 1
    Prision.map = 66
    Libertad.map = 66
...
On Error GoTo Errhandler
    ...
    Dim nfile As Integer
    nfile = FreeFile ' obtenemos un canal
    Open App.path & "\AntiCheat.log" For Append Shared As #nfile
    Print #nfile, Date & " " & Time & " " & Texto
    Print #nfile, ""
    Close #nfile
...
Private Function EstaAbrigado(ByVal userindex As Integer) As Boolean
    '***********************************************
    'Author: Nhelk
    'Last Modification: 1/05/2016
    'Is user using an invernal armour?
    '***********************************************
    EstaAbrigado = False
    With UserList(userindex).Invent
        If .ArmourEqpObjIndex <> 0 Then
            ''If (.ArmourEqpObjIndex > 832 And .ArmourEqpObjIndex < 836) Or (.ArmourEqpObjIndex > 664 And .ArmourEqpObjIndex < 668) Then
...
'***************************************************
    ...
    With UserList(userindex)
        If .Counters.Frio < IntervaloFrio Then
            .Counters.Frio = .Counters.Frio + 1
        Else
            If MapInfo(.Pos.map).Terreno = Nieve Or ((.Pos.map = 169 Or .Pos.map = 170 Or .Pos.map = 171) And Not EstaAbrigado(userindex)) Then
                Call WriteConsoleMsg(userindex, "¡¡Estás muriendo de frío, abrigate o morirás!!", FontTypeNames.FONTTYPE_INFO)
                modifi = Porcentaje(.Stats.MaxHp, 5)
                .Stats.MinHp = .Stats.MinHp - modifi
...
On Error GoTo Errhandler
    Dim i As Long
    Call mod_T1v1.PasaunSegundo
    Call mod_Retos1vs1.PasaSegundo
    Call cuentaRegresiva
    Call mod_DeathMatch.PassSecondDeath
    Call Event_1vs1_Aim_Melee.Count
    ''Call mod_Retos3vs3.Count_Reto
    Call CaptureTimer
    Call loop_reto
    For i = 1 To LastUser
        If UserList(i).flags.UserLogged Then
            Call mod_Retos2vs2.user_retoLoop(CInt(i))

            Call actualizarAntiCheat(i)
            With UserList(i).UserDeath
                If .SecondsBack > 1 Then
                    .SecondsBack = .SecondsBack - 1
                    WriteConsoleMsg i, "DeathMatch> Quedan " & .SecondsBack & " segundos para regresar a tu posición anterior", FontTypeNames.FONTTYPE_INFO
                End If
                If .SecondsBack = 1 Then
                    WarpUserChar i, .lastPos.map, .lastPos.X, .lastPos.Y, True
                    .SecondsBack = 0
            ...
            If UserList(i).Counters.Saliendo Then
                ...
                If UserList(i).Counters.Salir > 0 Then Call WriteConsoleMsg(i, "Cerrando...Se cerrará el juego en " & UserList(i).Counters.Salir & " segundos...", FontTypeNames.FONTTYPE_INFO)
...
Sub eventLoad()
        ...
        Call mod_Retos2vs2.initRetoData2
...
Sub eventClose(ByVal userindex As Integer)
        ...
        With UserList(userindex)
                ...
                If reto_List(.reto2Data.reto_Index).used_ring Then
                        Call mod_Retos2vs2.disconnect_Reto(userindex)
                End If
