...
On Error Resume Next
    ...
    Select Case sndRoute
        ...
        Case SendTarget.ToReto
            With Arena(sndIndex)
                If UserList(.Player(0).UserIndex).ConnID <> -1 Then
                    Call EnviarDatosASlot(.Player(0).UserIndex, sndData)
                End If
                If UserList(.Player(1).UserIndex).ConnID <> -1 Then
                    Call EnviarDatosASlot(.Player(1).UserIndex, sndData)
                End If
            End With
            
        Case SendTarget.ToRetoPlantes
            With Arena(sndIndex)
                If UserList(.Player(0).UserIndex).ConnID <> -1 Then
                    Call EnviarDatosASlot(.Player(0).UserIndex, sndData)
                End If
