    ...
    'Actualiza todos los slots
        For LoopC = 1 To .CurrentInventorySlots
            ...
            If .Invent.Object(LoopC).ObjIndex > 0 Then
                ...
                ''Debug.Print .Invent.Object(LoopC).ObjIndex
            Else
...
With UserList(UserIndex)
    If Num > 0 Then
        ...
        If MapData(.Pos.map, X, Y).ObjInfo.ObjIndex = 0 Or (MapData(.Pos.map, X, Y).ObjInfo.ObjIndex = Obj.ObjIndex And MapData(.Pos.map, X, Y).ObjInfo.Amount + Obj.Amount < MAX_INVENTORY_OBJS) Then
...
With MapData(map, X, Y)
    ...
    If .ObjInfo.Amount <= 0 Then
        ...
        Call mod_Limpieza.QuitarObj(map, X, Y)
...
On Error GoTo Errhandler
    ...
    With UserList(UserIndex)
        ...
        Select Case Obj.OBJType
                ...
                If ClasePuedeUsarItem(UserIndex, ObjIndex, sMotivo) And _
                   ...
                   'Si esta equipado lo quita
                    If .Invent.Object(Slot).Equipped Then
                        Call Desequipar(UserIndex, Slot)
                        Call DarCuerpoDesnudo(UserIndex, .flags.Mimetizado = 1)
                        If Not .flags.Mimetizado = 1 And .flags.AdminInvisible = 0 Then
                            Call ChangeUserChar(UserIndex, .Char.body, .Char.Head, .Char.heading, .Char.WeaponAnim, .Char.ShieldAnim, .Char.CascoAnim)
                        End If
                        Exit Sub
                    End If
                    ...
                    Else
                        ...
                        If .flags.AdminInvisible = 0 Then _
                        Call ChangeUserChar(UserIndex, .Char.body, .Char.Head, .Char.heading, .Char.WeaponAnim, .Char.ShieldAnim, .Char.CascoAnim)
                    End If
                    .flags.Desnudo = 0
                Else
            ...
            Case eOBJType.otESCUDO
                If .UserReto.EnReto = True Then If .UserReto.Escudos_Cascos = True Then Exit Sub
                If .flags.Navegando = 1 Then Exit Sub
...
Sub UseInvItem(ByVal UserIndex As Integer, ByVal Slot As Byte, ByVal Dck As Boolean)
'*************************************************
'Author: Unknown
...
'*************************************************
    ...
    With UserList(UserIndex)
        ...
        If Obj.OBJType = eOBJType.otWeapon Then
            ...
            End If
        End If
        If Not modAntiCheat.PuedeIntervalo(UserIndex, IIf(Dck = True, IntControl.UsarDck, IntControl.Usar)) Then Exit Sub
        ObjIndex = .Invent.Object(Slot).ObjIndex
            ...
            Case eOBJType.otPociones
                ...
                Select Case .flags.TipoPocion
                    ...
                    Case 4 'Pocion azul, restaura MANA
                        ...
                        .Stats.MinMAN = .Stats.MinMAN + Porcentaje(.Stats.MaxMAN, 5) '' + .Stats.ELV \ 2 + 40 / .Stats.ELV
                    ...
                    Case 7
                        .Stats.MinSta = .Stats.MinSta + (.Stats.MinSta * 0.05)
