...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        SendData SendTarget.ToPCArea, UserIndex, PrepareMessageCreateDamage(UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y, "-" & daño, 255, 0, 0)
...
On Error GoTo Errhandler
    ...
    With UserList(AtacanteIndex)
        ...
        If .flags.Hambre = 0 And .flags.Sed = 0 Then
            ...
            Else
                SendData SendTarget.ToPCArea, VictimaIndex, PrepareMessageCreateDamage(UserList(VictimaIndex).Pos.X, UserList(VictimaIndex).Pos.Y, "-" & daño, 255, 0, 0)
            End If
...
On Error GoTo Errhandler
    ...
    If Not eventAttack(attackerIndex, victimIndex) Then
        ...
        Exit Function
    End If
    If UserList(attackerIndex).UserDeath.EnDeath = True Then
        If Not death_PuedeAtacar(attackerIndex) Then
            PuedeAtacar = False
            Exit Function
        End If
    End If
    If UserList(attackerIndex).HungerGames.HungerGamers = True Then
        If Not mod_HungerGames.HungerGames_CanAttack(attackerIndex) Then
            PuedeAtacar = False
            Exit Function
        End If
    End If
...
'***************************************************
    ...
    If ExpaDar > 0 Then
        If UserList(UserIndex).PartyIndex > 0 Then
            'PARTY 9010
            Call mod_Party.GanaExperiencia(ExpaDar, UserIndex)
        Else
