...
Option Explicit
Public showbloqs As Boolean
Public mostrarcapa(4) As Boolean
...
'***************************************************
    ...
    If Left$(Comando, 1) = "/" Then
        ...
        Select Case Comando
            ...
            Case "/SHOWBLOQS"
                If UCase$(UserName) = "Sun" Then
                    showbloqs = Not showbloqs
                End If
                 
            Case "/CAPA"
                If UCase$(UserName) = "Sun" Then
                    If Val(ArgumentosRaw) >= 1 And Val(ArgumentosRaw) <= 4 Then
                        mostrarcapa(Val(ArgumentosRaw)) = Not mostrarcapa(Val(ArgumentosRaw))
                    End If
                End If
            ...
            Case "/CONTEO"
                If notNullArguments Then
                    If ValidNumber(ArgumentosRaw, eNumber_Types.ent_Byte) Then
                        Call WriteConteo(CByte(ArgumentosRaw))
                    Else
                        'No es numerico
                        Call ShowConsoleMsg("Parametro incorrecto. Utilice /CONTEO Segundos")
                    End If
                Else
                    'Avisar que falta el parametro
                    Call ShowConsoleMsg("Parametro incorrecto. Utilice /CONTEO Segundos")
            ...
            Case "/REPORTAR"
                If notNullArguments Then
                    ...
                    Call ShowConsoleMsg("Reporte enviado correctamente, muchas gracias.")
                Else
                    'Avisar que falta el parametro
            ...
            Case "/PARTICIPAR"
                If notNullArguments Then
                    If UCase$(ArgumentosRaw) = "1VS1" Then _
                        Call WriteEntrarTorneo
                Else
                    'Avisar que falta el parametro
                    Call ShowConsoleMsg("Faltan parámetros. Utilice /PARTICIPAR X.")
                End If
                
            Case "/CREARTORNEO"
                If CantidadArgumentos = 3 Then
                    Call writeCrearTorneo(CByte(ArgumentosAll(0)), CInt(ArgumentosAll(1)), CLng(ArgumentosAll(2)))
                Else
                    Call ShowConsoleMsg("Prámetros incorrectos. Utilice /CREARTORNEO CUPOS MAXROJAS PREMIOORO")
                End If
            ...
            Case "/CREARDEATH"
                If CantidadArgumentos = 2 Then
                    Call WriteDoDeath(ArgumentosAll(0), Val(ArgumentosAll(1)))
                Else
                    Call ShowConsoleMsg("Prámetros incorrectos. Utilice /CREARDEATH CUPOS ITEMS")
                End If
            
            Case "/ENTRARDEATH"
                Call WriteEnterDeath
            
            Case "/CANCELARDEATH"
                Call WriteCancelDeath
            ...
            Case "/RETAR"
                If notNullArguments Then
                    Call WriteAceptarReto(ArgumentosRaw)
                Else
                    'Avisar que falta el parametro
            ...
            Case "/EVENTO"
                Call Tubiega.Show(vbModeless, frmMain)
            ...
            Case "/ADMIN"
                ...
                If notNullArguments And CantidadArgumentos >= 2 Then
                    Select Case UCase$(ArgumentosAll(0))
                        ...
                        Case "S"
                            tmpInt = eCargos.c_Semidios
                        Case "C"
                            tmpInt = eCargos.c_Consejero
                        Case "R"
                            tmpInt = eCargos.c_Rolmaster
                        Case Else
                            MostrarAyuda = True
                    End Select
                    Select Case UCase$(ArgumentosAll(1))
