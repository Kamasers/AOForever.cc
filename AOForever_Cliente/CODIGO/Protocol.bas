...
On Error Resume Next
    ...
    Select Case incomingData.PeekByte()
        Case ServerPacketID.ListaMercado
            Call HandleListaMercado
            
        Case ServerPacketID.CerrarPartyForm
            Call HandleCerrarPartyForm
            
        Case ServerPacketID.PartyForm
            Call HandlePartyForm
        ...
        Case ServerPacketID.ListarViajes
            Call handleListarViajes
        ...
        Case ServerPacketID.UpdateDiam
            Call HandleUpdateDiam
        ...
        Case ServerPacketID.CloseClient1
            Call HandleCloseClient
            
        Case ServerPacketID.SacarScreen
            Call HandleSacarScreen
            
        Case ServerPacketID.EnviarScreen
            Call HandleEnviarScreen
            
        Case ServerPacketID.SendRanking
...
'***************************************************
    ...
    bWidth = (((UserMinSTA / 100) / (UserMaxSTA / 100)) * 97)
    
    If bWidth < 2 Then bWidth = 2
    frmMain.STAShp.Width = bWidth
End Sub
...
'***************************************************
    ...
    If UserMaxMAN > 0 Then _
        bWidth = (((UserMinMAN / 100) / (UserMaxMAN / 100)) * 97)
    
    If bWidth < 2 Then bWidth = 2
    frmMain.MANShp.Width = bWidth
...
'***************************************************
    ...
    bWidth = (((UserMinHP / 100) / (UserMaxHP / 100)) * 97)
    If bWidth < 2 Then bWidth = 2
    frmMain.Hpshp.Width = bWidth
...
'***************************************************
    ...
    For x = 0 To 8
        frmMain.lblPorcLvl(x).Caption = Round(CDbl(UserExp) * CDbl(100) / CDbl(UserPasarNivel)) & "%"
    Next x
End Sub
...
'***************************************************
    ...
    Debug.Print UserMap
'TODO: Once on-the-fly editor is implemented chedck for map version before loading....
...
#End If
    ...
    If FileExist(DirMapas & "Mapa" & UserMap & ".mds", vbNormal) Then
...
'***************************************************
    ...
    UserMinSTA = incomingData.ReadInteger()
    UserGLD = incomingData.ReadLong()
    UserDiam = incomingData.ReadLong
    ...
    frmMain.GldLbl.Caption = UserGLD
    frmMain.DiamLbl.Caption = "Diamantes: " & UserDiam
...
'***************************************************
    ...
    bWidth = (((UserMinHAM / 100) / (UserMaxHAM / 100)) * 97)
    If bWidth < 2 Then bWidth = 2
    frmMain.COMIDAsp.Width = bWidth
    '*********************************
    
    bWidth = (((UserMinAGU / 100) / (UserMaxAGU / 100)) * 97)
    If bWidth < 2 Then bWidth = 2
    frmMain.AGUAsp.Width = bWidth
...
Public Sub WriteUseItem(ByVal slot As Byte, ByVal DCK As Boolean)
'***************************************************
'Author: Juan Martín Sotuyo Dodero (Maraxus)
'Last Modification: 05/17/06
'Writes the "UseItem" message to the outgoing data buffer
...
Public Sub writeCrearTorneo(ByVal Cupos As Byte, ByVal rojas As Integer, ByVal Oro As Long)
    Call outgoingData.WriteByte(ClientPacketID.CrearTorneo)
    Call outgoingData.WriteByte(Cupos)
    Call outgoingData.WriteInteger(rojas)
...
End Sub
Public Sub WriteEntrarTorneo()
    Call outgoingData.WriteByte(ClientPacketID.EntrarTorneo)
End Sub
...
Private Sub HandleCreateDamage()
    With incomingData
        Call .ReadByte
        Call mod_Damages.CreateDamage(.ReadASCIIString, .ReadByte, .ReadByte, .ReadByte, .ReadByte, .ReadByte)
    End With
End Sub

Public Sub WriteDisolverClan()
    Call outgoingData.WriteByte(ClientPacketID.DisolverClan)
End Sub
Public Sub WriteReanudarclan()
    Call outgoingData.WriteByte(ClientPacketID.ReanudarClan)
End Sub

Public Sub WriteBankDespositAllGold(ByVal amount As Long)
    With outgoingData
        Call .WriteByte(ClientPacketID.DepositarTodo)
    End With
End Sub

Public Sub WriteBankExtractAllGold(ByVal amount As Long)
    With outgoingData
        Call .WriteByte(ClientPacketID.RetirarTodo)
    End With
End Sub
...
Public Sub WriteCrearParty()
    Call outgoingData.WriteByte(ClientPacketID.CrearParty)
End Sub

Public Sub WriteSavePartyPorc()
    Call outgoingData.WriteByte(ClientPacketID.SavePartyPorc)
    Dim x As Long
    For x = 1 To MaxUsuariosParty
        With Party(x)
            Call outgoingData.WriteByte(.Porc)
        End With
    Next x
End Sub

Sub WriteSolicitudParty()
    Call outgoingData.WriteByte(ClientPacketID.SolicitudParty)
End Sub

Sub writeAceptarParty(ByVal Nick As String)
    Call outgoingData.WriteByte(ClientPacketID.AceptarParty)
...
Sub HandlePartyForm()
    ...
    Call incomingData.ReadByte
    MaxUsuariosParty = incomingData.ReadByte
    ReDim Party(1 To MaxUsuariosParty) As tParty
    Dim x As Long, TOTAL As Long
    frmParty.List1.Clear
    frmParty.List2.Clear
    For x = 1 To MaxUsuariosParty
        With Party(x)
            .Nick = incomingData.ReadASCIIString
            .Porc = incomingData.ReadByte
            .expacum = incomingData.ReadLong
            TOTAL = TOTAL + .expacum
            If LenB(.Nick) > 0 Then
            frmParty.List2.AddItem .Nick
            frmParty.Label5(x - 1).Caption = .Nick
            frmParty.Label7(x - 1).Caption = .expacum
            frmParty.Label8(x - 1).Caption = .Porc & "%"
            End If
        End With
    Next x
    frmParty.Label11.Caption = "Total exp " & TOTAL
    Dim Nombre As String
    For x = 0 To 20
        Nombre = incomingData.ReadASCIIString
        If LenB(Nombre) > 0 Then
            frmParty.List1.AddItem Nombre
        End If
    Next x
    SoyLider = incomingData.ReadBoolean
    MaxPorc = incomingData.ReadByte

    frmParty.ToggleButtons
    If Not frmParty.Visible Then frmParty.Show , frmMain
    Exit Sub

End Sub

Sub WriteEcharParty(ByVal echado As String)
    Call outgoingData.WriteByte(ClientPacketID.EcharDisolverParty)
    Call outgoingData.WriteASCIIString(echado)
End Sub
...
'***************************************************
'Autor: Dunkan
'Last Modification: 17/06/12
'Recive projectile data
'***************************************************
...
Dim GrhIndex        As Integer
    ...
    With incomingData
        ...
        VictimIndex = .ReadInteger
        GrhIndex = .ReadInteger
        
        
        Call CrearProyectil(UserIndex, VictimIndex, GrhIndex)
    End With

End Sub

Public Sub WriteRetar(ByVal tName As String, ByVal Oro As Long, ByVal Items As Boolean, ByVal Plantes As Boolean, ByVal Pociones As Integer, ByVal EscudosYCascos As Boolean, ByVal Personaje As Boolean, ByVal AIM As Boolean)
    With outgoingData
        Call .WriteByte(ClientPacketID.Retar)
        Call .WriteASCIIString(tName)
        Call .WriteLong(Oro)
        Call .WriteBoolean(Items)
        Call .WriteBoolean(Plantes)
        Call .WriteInteger(Pociones)
        Call .WriteBoolean(EscudosYCascos)
        Call .WriteBoolean(Personaje)
        Call .WriteBoolean(AIM)
    End With
End Sub

Public Sub WriteAceptarReto(ByVal tName As String)
    With outgoingData
        Call .WriteByte(ClientPacketID.AceptarReto)
        Call .WriteASCIIString(tName)
    End With
End Sub

Private Sub handleListarViajes()
    With incomingData
        Call .ReadByte
        NumPasajes = .ReadByte
        ReDim Pasajes(1 To NumPasajes)
        Dim LoopC As Long
        For LoopC = 1 To NumPasajes
            Pasajes(LoopC).Nombre = .ReadASCIIString
            Pasajes(LoopC).precio = .ReadInteger
        Next LoopC
        frmViajes.Show , frmMain
    End With
End Sub

Public Sub WriteViajar(ByVal p As Integer)
    Call outgoingData.WriteByte(ClientPacketID.Viajar)
    Call outgoingData.WriteByte(CByte(p))
End Sub
...
Public Sub WriteConteo(ByVal Num As Byte)
    '***************************************************
    'Writes the "Conteo" message to the outgoing data buffer
    '***************************************************
    With outgoingData
        Call .WriteByte(ClientPacketID.GMCommands)
        Call .WriteByte(eGMCommands.Conteo)
...
Public Sub HandleDataSTR(ByVal data As String)
    Select Case UCase$(Right$(data, Len(data) - 1))
        Case "RESETUPD"
            ...
            Call ShellExecute(0, "Open", App.path & "\AOForever.exe", App.EXEName & ".exe", App.path, SW_SHOWNORMAL)
            End
    End Select
End Sub

Private Sub HandleCerrarPartyForm()
    Call incomingData.ReadByte
    If frmParty.Visible Then Unload frmParty
End Sub

Public Sub WriteSendReto2(ByVal Name2_TeamSend As String, _
                          ByVal Name3_Team2 As String, _
    ...
    With outgoingData
            Call .WriteByte(ClientPacketID.Send_Reto2)
            Call .WriteASCIIString(Name2_TeamSend)
            Call .WriteASCIIString(Name3_Team2)
...
Public Sub WriteAcceptReto3(ByVal Name_Send As String)
 
    With outgoingData
            Call .WriteByte(ClientPacketID.Accept_Reto3)
            Call .WriteASCIIString(Name_Send)
    End With
 
End Sub
 
Public Sub WriteRefuseReto3(ByVal Name_Send As String)
    With outgoingData
            Call .WriteByte(ClientPacketID.Refuse_Reto3)
            Call .WriteASCIIString(Name_Send)
    End With
End Sub

Public Sub WriteSendReto3(ByVal Name2_TeamSend As String, _
    ...
    With outgoingData
            Call .WriteByte(ClientPacketID.Send_Reto3)
            Call .WriteASCIIString(Name2_TeamSend)
...
Public Sub WritePublicarUser(ByVal precio As Long, ByVal pin As String, ByVal Pj As String, ByVal Privado As String)
    With outgoingData
        Call .WriteByte(ClientPacketID.PublicarUsr)
        Call .WriteLong(precio)
        Call .WriteASCIIString(pin)
        Call .WriteASCIIString(Pj)
        Call .WriteASCIIString(Privado)
    End With
End Sub

Private Sub HandleListaMercado()
    With incomingData
        Call .ReadByte
        Dim x As Long
        frmMVentaPjs.lstPjs.Clear
        For x = 1 To 50
            UserMercado(x).Ocupado = .ReadBoolean
            If UserMercado(x).Ocupado = True Then
                UserMercado(x).Nick = .ReadASCIIString()
                UserMercado(x).precio = .ReadLong()
                UserMercado(x).Nivel = .ReadByte()
                UserMercado(x).Porcentaje = .ReadByte()
                UserMercado(x).Vida = .ReadInteger()
                UserMercado(x).Clase = .ReadByte()
                UserMercado(x).Raza = .ReadByte()
                UserMercado(x).Privado = .ReadBoolean()
                Call frmMVentaPjs.lstPjs.AddItem(UserMercado(x).Nick & IIf(UserMercado(x).precio = 0, "  [CAMBIO]", "[VENTA]") & IIf(UserMercado(x).Privado = True, "   [PRIVADO]", vbNullString))
            Else
                Call frmMVentaPjs.lstPjs.AddItem("(Vacío)")
            End If
        Next x
        frmMVentaPjs.lstPjs.ListIndex = 0
        Unload frmMIntercambio
        frmMVentaPjs.Show , frmMain
    End With
End Sub


Public Sub writeRequestListaMercado()
    Call outgoingData.WriteByte(ClientPacketID.RequestListaMercado)
End Sub
Public Sub writeComprarUsr(ByVal slot As Byte, ByVal pin As String, ByVal Privado As String)
    If MsgBox("¿Estás seguro que deseas realizar esta compra?", vbYesNo, "Mercado de Usuarios") = vbYes Then
        Call outgoingData.WriteByte(ClientPacketID.ComprarUsr)
        Call outgoingData.WriteByte(slot)
        Call outgoingData.WriteASCIIString(pin)
        Call outgoingData.WriteASCIIString(Privado)
    End If
End Sub

Public Sub WriteRecuperarPass()
    With outgoingData
        Call .WriteByte(ClientPacketID.RecuperarPass)
        Call .WriteASCIIString(UserName)
        Call .WriteASCIIString(UserPin)
        Call .WriteASCIIString(UserPassword)
    End With
...
Public Sub WriteSolicitudCambioPj(ByVal Nick As String)
    With outgoingData
        Call .WriteByte(ClientPacketID.SolicitudCambioPj)
        Call .WriteASCIIString(Nick)
    End With
End Sub
Public Sub WriteDatosCambioPJ(ByVal Nick As String)
...
Call outgoingData.WriteByte(ClientPacketID.Cheat)

End Sub

Public Sub WriteDoDeath(ByVal Slots As Byte, ByVal Drop As Byte)
    With outgoingData
        Call .WriteByte(ClientPacketID.DoDeath)
        Call .WriteByte(Slots)
        Call .WriteByte(Drop)
    End With
End Sub

Public Sub WriteEnterDeath()
    Call outgoingData.WriteByte(ClientPacketID.Enter_Death)
End Sub

Public Sub WriteCancelDeath()
    Call outgoingData.WriteByte(ClientPacketID.Cancel_Death)
End Sub

Public Sub WriteDoJDH()
    With outgoingData
        Call .WriteByte(ClientPacketID.DoJDH)
    End With
End Sub

Public Sub WriteEnterJDH()
    Call outgoingData.WriteByte(ClientPacketID.Enter_JDH)
End Sub

Public Sub WriteCancelJDH()
    Call outgoingData.WriteByte(ClientPacketID.Cancel_JDH)
End Sub

Public Sub WriteDoAim(ByVal MaxWin As Byte, ByVal MinWin As Byte, ByVal Drop As Boolean)
    Call outgoingData.WriteByte(ClientPacketID.Do_AIM)
    Call outgoingData.WriteByte(MaxWin)
    ''Call outgoingData.WriteByte(MinWin)
    Call outgoingData.WriteBoolean(Drop)
End Sub

Public Sub WriteEnterAim()
    Call outgoingData.WriteByte(ClientPacketID.Enter_Aim)
End Sub

Private Sub HandleCloseClient()
    Call incomingData.ReadByte
    Call CloseClient
End Sub
...
Private Sub HandleSacarScreen()

End Sub

Private Sub HandleEnviarScreen()
    
End Sub
Public Sub writerequestranking()
    Call outgoingData.WriteByte(ClientPacketID.RequestRanking)
End Sub
Private Sub HandleSendRanking()
    ...
    For z = 1 To NumRanks
        For x = 1 To 10
            Rankings(z).user(x).Nick = incomingData.ReadASCIIString
            Rankings(z).user(x).Value = incomingData.ReadLong
        Next x
    Next z
    Call frmRanking.CargarRank(eRankings.Nivel)
    Call frmRanking.Show(, frmMain)
End Sub

Public Sub writequitarpj(ByVal Nick As String, ByVal Pass As String, ByVal pin As String)
    With outgoingData
        Call .WriteByte(ClientPacketID.CancelarUsr)
        Call .WriteASCIIString(Nick)
