...
Option Explicit
Public MovimientoHablar As Boolean
...
Public vWrite As String
Public bFogata As Boolean

Public bLluvia() As Byte ' Array para determinar si
'debemos mostrar la animacion de la lluvia
Const inputCancel As String = "{[(CANCEL)]}"
Private lFrameTimer As Long

Public Function CustomInput(ByVal Title As String) As String
    frmCustomInput.Caption = Title
    frmCustomInput.Show vbModal, frmMain
    CustomInput = vWrite
End Function
...
'TODO : Si hay que reinstalar, porque no cierra???
    ...
    ' Atacable
    ColoresPJ(48).r = Val(GetVar(archivoC, "AT", "R"))
    ColoresPJ(48).g = Val(GetVar(archivoC, "AT", "G"))
    ColoresPJ(48).b = Val(GetVar(archivoC, "AT", "B"))
End Sub
...
#End If
    ...
    MaxAlpha = 255
    If UserName = "Arzhen" Or UserName = "Arzen" Then
        editado = True
    Else
        editado = False
    End If
...
'*****************************************************************
    ...
    Else
        Exit Sub
    End If
    If ((frmMain.SendTxt.Visible) Or (frmMain.SendCMSTXT.Visible)) And Not MovimientoHablar Then Exit Sub
...
Sub SwitchMap(ByVal Map As Integer)
     ...
     dLen = FileLen(DirMapas & "Mapa" & Map & ".mds")
     ReDim dData(dLen - 1)
 
     lngstart = GetTickCount
     handle = FreeFile()
 
     Open DirMapas & "Mapa" & Map & ".mds" For Binary As handle
     'Seek handle, 1
      Get handle, , dData
      Close handle
...
Sub Main()
    ...
    Call CheckLauncherUpdate
    frmCargando.Show
    frmCargando.Refresh
    
    
    #If Desarrollo = 0 Then
        If Not LeerInt(App.path & "\INIT\Launcher.ini") = 1 Then
            MsgBox "Debes abrir el juego desde AOForever.exe"
            End
        End If
    #End If
    Call GuardarInt(App.path & "\INIT\Launcher.ini", 0)
    #If Desarrollo = 0 Then
        Ip = "127.0.0.1" '
        port = 7666
    #Else
    ...
    mod_setup.LoadIni
    If tieneReg = True Then End
    DoEvents
    frmCargando.NewPercentage 10
...
'TODO : esto de ServerRecibidos no se podría sacar???
    ...
    frmCargando.NewPercentage 40
    If Not InitTileEngine(frmMain.hwnd, 149, 7, 32, 32, 13, 17, 6, 7, 7, 0.02) Then
        Call CloseClient
    End If
   ...
   'Init timers
    ...
    Do While prgRun
        If GetTickCount <= 0 Then
            MsgBox "Hubo un error con la función 'GetTickCount', tu pc estuvo prendida por mas de 49.7 dias. Podes solucionarlo reiniciando tu pc."
            Exit Do
        End If
...
Public Sub CheckLauncherUpdate()
    If FileExist(App.path & "\Extras\LauncherNew.exe", vbNormal) = True Then
        Call Kill(App.path & "\AOForever.exe")
        Call FileCopy(App.path & "\Extras\LauncherNew.exe", App.path & "\AOForever.exe")
        If FileExist(App.path & "\Extras\LauncherNew.exe", vbNormal) = True Then
            Call Kill(App.path & "\Extras\LauncherNew.exe")
        End If
    End If
End Sub
